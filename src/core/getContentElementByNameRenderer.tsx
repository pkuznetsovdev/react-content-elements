...
import { CONTENT_ELEMENTS_BY_NAME } from "./content-element";
...
>(
  ...
  return (contentElementName: ElementName) => {
    return (props: ContentElementProps1<ElementName> & CustomProps) => {
      ...
      const ContentElementTemplate = elementTemplatesByNameWithDefaultValues[
        contentElementName
      ] as React.FC<ContentElementTemplateProps<ElementName>>;

      if (!ContentElementTemplate) {
        return null;
      }

      const contentElementConfig = getConfigByValidatedProps(
        propsWithoutCEIf,
        contentElementName,
      );

      return WithContentElementConfig(ContentElementTemplate)(
        // TODO: FAQ HOW TO FIX?
        // @ts-ignore
...
function getContentElementConfigFromProps<
  ElementName extends ContentElementName,
>(
  props: Omit<ContentElementProps1<ElementName>, "if">,
  contentElementName: ElementName,
